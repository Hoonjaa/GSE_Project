...
{
    if (m_composite && m_blur) glGenVertexArrays(1, &m_vao);
}
PostProcessing::~PostProcessing()
{
    ...
    if (m_blur) glDeleteProgram(m_blur);
}
void PostProcessing::ReleaseTargets()
{
    m_ready = false;
    if (m_sceneFbo) glDeleteFramebuffers(1, &m_sceneFbo);
    if (m_sceneColor) glDeleteRenderbuffers(1, &m_sceneColor);
    if (m_sceneDepth) glDeleteRenderbuffers(1, &m_sceneDepth);
...
void PostProcessing::Resize(int width, int height)
{
    ...
    if (!m_composite || !m_blur || !m_vao) return;
    ...
    bool ok = width <= maxTexture && height <= maxTexture &&
        width <= maxRenderbuffer && height <= maxRenderbuffer;
    if (ok) ok = MakeTarget(m_resolveFbo, m_resolveColor, width, height);
    if (ok) ok = MakeTarget(m_blurFbo[0], m_blurColor[0], m_halfWidth, m_halfHeight);
    if (ok) ok = MakeTarget(m_blurFbo[1], m_blurColor[1], m_halfWidth, m_halfHeight);
    if (ok) {
        ...
        for (;;) {
            ...
            if (m_samples > 1) glRenderbufferStorageMultisample(GL_RENDERBUFFER, m_samples, GL_RGBA16F, width, height);
            ...
            if (m_samples > 1) glRenderbufferStorageMultisample(GL_RENDERBUFFER, m_samples, GL_DEPTH_COMPONENT24, width, height);
...
void PostProcessing::Composite(const PostProcessingSettings& settings)
{
    ...
    glUniform1f(glGetUniformLocation(m_composite, "u_Vignette"), settings.vignette ? settings.vignetteStrength : 0);
