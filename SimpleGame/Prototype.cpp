...
namespace
{
    ...
    {
        Mesh m;
        const Vec3 top[]={{0,.6f,-2.05f},{.85f,.6f,-.85f},{.85f,.6f,1.35f},
            {0,.6f,1.8f},{-.85f,.6f,1.35f},{-.85f,.6f,-.85f}};
        for (int i=0;i<6;++i) {
            Vec3 a=top[i],b=top[(i+1)%6];
            Triangle(m,{0,.6f,0},a,b,{.79f,.59f,.32f});
            Quad(m,a,b,{b.x*.65f,.02f,b.z*.85f},{a.x*.65f,.02f,a.z*.85f},
        ...
        for (int i=0;i<8;++i) {
            ...
            Quad(m,{x,4.24f,-.13f},{X,4.24f,-.13f},
...
{
    m_player=WorldPosition();m_player.x=22;m_player.z=17.75;
    m_ship=WorldPosition();m_ship.x=29;m_ship.z=20.3;
    m_camera=m_player;m_sailing=false;m_heading=0;m_shipHeading=0;
    m_keys.fill(false);
    m_message=KoreanText::Welcome;
    m_messageTime=10;
}
void Prototype::ClearInput() { m_keys.fill(false); }
void Prototype::Key(unsigned char key,bool down)
{
    ...
    case 'g': m_grid=!m_grid; break;
    case 'h': m_hud=!m_hud; break;
    case 'r': Reset(); break;
    case 'p':
        ...
        m_messageTime=5; break;
    case '1': m_postSettings.toneMapping=!m_postSettings.toneMapping; break;
    case '2': m_postSettings.vignette=!m_postSettings.vignette; break;
    case '3': m_postSettings.edgeBlur=!m_postSettings.edgeBlur; break;
    case '[': m_postSettings.exposure=(std::max)(.1f,m_postSettings.exposure-.1f); break;
    case ']': m_postSettings.exposure=(std::min)(4.f,m_postSettings.exposure+.1f); break;
    case '0': m_postSettings=PostProcessingSettings(); break;
...
void Prototype::Interact()
{
    if (!m_sailing) {
        ...
        } else m_message=KoreanText::TooFarToBoard;
    } else {
        ...
        for (float distance=1.6f;distance<=3.25f && !landed;distance+=.2f)
            for (int i=0;i<32 && !landed;++i) {
                ...
                if (m_world.CanWalk(candidate)) {
                    m_player=candidate;m_sailing=false;landed=true;
                }
            }
...
void Prototype::Move(double dx,double dz)
{
    ...
    auto tryStep=[&](double x,double z) {
        ...
        auto next=position;next.Move(x,z);
    ...
    for (int i=0;i<steps;++i) {
        if (tryStep(dx,dz)) m_moving=true;
        else {
...
{
    if (dt>0) m_frameAverage+=(dt-m_frameAverage)*.05;
    ...
    if (length>0) {
        ...
        if (m_sailing) m_shipHeading=m_heading;
...
void Prototype::Render(Renderer& r)
{
    ...
    if (std::abs(shipOffset.x)<500 && std::abs(shipOffset.z)<500) {
        ...
        if (m_sailing && m_moving) {
            for (int n=0;n<6;++n) {
                ...
                Quad(water,{-spread,.008f,z},{spread,.008f,z},
                    {spread+.10f,.008f,z+.09f},{-spread-.1f,.008f,z+.09f},{.34f,.70f,.73f});
            }
        }
        r.Submit(water,shipOffset,m_shipHeading);
        auto bob=shipOffset;bob.y=.045f*std::sin(m_time*2);
        r.Submit(m_shipMesh,bob,m_shipHeading);
    }
    if (!m_sailing) {
        auto p=m_player.RelativeTo(m_camera);p.y=.58f;
        Mesh marker;
    ...
    if (std::abs(h.x)<90 && std::abs(h.z)<90) {
        ...
        Triangle(flag,{h.x+2.85f,3.9f,h.z-.3f},{h.x+4.1f,3.55f,h.z-.3f+std::sin(m_time*3)*.15f},
            {h.x+2.85f,3.2f,h.z-.3f},{.88f,.45f,.26f});
        r.Submit(flag);
    }
    ...
    if (m_hud) {
        ...
        postStatus<<KoreanText::PostLabel
            <<(r.PostProcessingAvailable()?(m_postSettings.enabled?KoreanText::On:KoreanText::Off):KoreanText::Unavailable)
            <<KoreanText::Exposure<<std::fixed<<std::setprecision(1)<<m_postSettings.exposure;
        ...
        effectStatus<<KoreanText::ToneLabel<<(m_postSettings.toneMapping?KoreanText::On:KoreanText::Off)
            <<KoreanText::VignetteLabel<<(m_postSettings.vignette?KoreanText::On:KoreanText::Off)
            <<KoreanText::BlurLabel<<(m_postSettings.edgeBlur?KoreanText::On:KoreanText::Off);
        ...
        if (W>760) {
            ...
            for (const auto& entry:m_world.Chunks()) {
                ...
                if (!island.hasIsland) continue;
                WorldPosition p;p.chunkX=entry.first.first;p.chunkZ=entry.first.second;
                p.x=island.centerX;p.z=island.centerZ;
                auto v=p.RelativeTo(ActivePosition());
                float x=left+86+v.x*1.1f,z=top+114+v.z*1.1f;
                float size=island.radius*1.1f;
                if (x-size<left+7 || x+size>left+165 || z-size<top+35 || z+size>top+193) continue;
                r.Panel(x-size,z-size,size*2,size*2,{.79f,.75f,.49f});
                r.Panel(x-size*.7f,z-size*.7f,size*1.4f,size*1.4f,
        ...
        status<<(m_sailing?KoreanText::Sailing:KoreanText::Walking)<<KoreanText::Chunk<<active.chunkX<<" : "<<active.chunkZ;
        ...
        debug<<KoreanText::Seed<<World::Seed<<KoreanText::LoadedChunks<<m_world.Chunks().size()
            <<KoreanText::Triangles<<r.TriangleCount()<<" / "<<static_cast<int>(1/m_frameAverage)<<KoreanText::Frames;
