...
namespace
{
    ...
    {
        return p.chunkX == 0 && p.chunkZ == 0 &&
            p.x >= 21 + margin && p.x <= 30 - margin &&
            p.z >= 17 + margin && p.z <= 18.5 - margin;
    }
    struct House { float x, z, w, d, h; Color roof; };
    const House Houses[] = {
    ...
    void HouseMesh(Mesh& m, const House& h)
    {
        ...
        float X=x+w+.2f,Z=z+d+.2f; x-=.2f; z-=.2f;
        Quad(m,{x,y,z},{X,y,z},{X,y+1,z+d*.5f+.2f},{x,y+1,z+d*.5f+.2f},h.roof);
        Quad(m,{x,y+1,z+d*.5f+.2f},{X,y+1,z+d*.5f+.2f},{X,y,Z},{x,y,Z},
...
void WorldPosition::Move(double dx,double dz)
{
    ...
    auto axis=[](std::int64_t& chunk,double& local,double delta) {
        ...
        if (!std::isfinite(next) || std::abs(delta)>ChunkSize*1000) return;
        ...
        if ((shift>0 && chunk>limit-shift) || (shift<0 && chunk< -limit-shift)) return;
        chunk+=shift; local=next-static_cast<double>(shift)*ChunkSize;
    };
    axis(chunkX,x,dx); axis(chunkZ,z,dz);
}
Vec3 WorldPosition::RelativeTo(const WorldPosition& origin) const
{
    ...
    auto relative=[](std::int64_t a,std::int64_t b,double local) {
        ...
        const std::uint64_t distance=a>=b ? std::uint64_t(a)-std::uint64_t(b) :
            std::uint64_t(b)-std::uint64_t(a);
        if (distance>1000) return a>=b ? 100000.f : -100000.f;
        return static_cast<float>((a>=b?1.0:-1.0)*double(distance)*ChunkSize+local);
    };
    return {relative(chunkX,origin.chunkX,x-origin.x),0,
        relative(chunkZ,origin.chunkZ,z-origin.z)};
}
...
{
    std::uint64_t state=Mix(Seed ^ Mix(static_cast<std::uint64_t>(cx)) ^
        (Mix(static_cast<std::uint64_t>(cz)+1337)*0x9e3779b97f4a7c15ULL));
    IslandChunk c;
    c.harbor=(cx==0 && cz==0);
    c.hasIsland=c.harbor || Random(state)>.30f;
    if (!c.hasIsland) return c;
    c.centerX=13+Random(state)*6; c.centerZ=13+Random(state)*6;
    c.radius=5+Random(state)*3.5f; c.phase=Random(state)*2*Pi;
    c.ruin=Random(state)>.7f;
    if (c.harbor) {c.centerX=16;c.centerZ=16;c.radius=9;c.phase=0;c.ruin=false;}
    ...
    for (int ring=0;ring<4;++ring) {
        for (int i=0;i<64;++i) {
            ...
            Triangle(c.terrain,{c.centerX,heights[ring],c.centerZ},
                {c.centerX+std::cos(a)*ra,heights[ring],c.centerZ+std::sin(a)*ra},
                {c.centerX+std::cos(b)*rb,heights[ring],c.centerZ+std::sin(b)*rb},colors[ring]);
        }
    }
    ...
    if (c.harbor) {
        Quad(c.scenery,{12,Ground+.022f,17},{23,Ground+.022f,17},
            {23,Ground+.022f,18.5f},{12,Ground+.022f,18.5f},{.84f,.74f,.51f});
        for (const auto& h:Houses) HouseMesh(c.scenery,h);
        for (int i=0;i<28;++i)
            Box(c.scenery,{21+i*.32f,.37f,17},{.29f,.18f,1.5f},{.59f,.39f,.22f});
        for (int i=0;i<5;++i)
            for (float z:{17.05f,18.25f})
                Box(c.scenery,{22+i*1.7f,-.15f,z},{.18f,1.02f,.18f},{.38f,.25f,.16f});
        for (int i=0;i<3;++i)
            Box(c.scenery,{21.f+i*.65f,Ground,16.2f},{.55f,.55f,.55f},{.64f,.44f,.24f});
        // Lighthouse.
        Box(c.scenery,{19.8f,Ground,12.5f},{1.4f,3.8f,1.4f},{.93f,.88f,.70f});
        Box(c.scenery,{19.72f,3.1f,12.42f},{1.56f,.45f,1.56f},{.82f,.32f,.22f});
        Box(c.scenery,{19.95f,4.35f,12.65f},{1.1f,.75f,1.1f},{.99f,.77f,.32f});
        Cone(c.scenery,{20.5f,5.1f,13.2f},1.15f,.8f,{.19f,.34f,.40f},4);
        Palm(c.scenery,11,21,3.1f,0); Palm(c.scenery,14,9,3.4f,1);
        Palm(c.scenery,22,16,2.9f,2); Palm(c.scenery,18,23,3.1f,3);
    } else {
        for (int i=0;i<7;++i) {
            const float a=Random(state)*2*Pi,r=(.25f+Random(state)*.4f)*c.radius;
            Palm(c.scenery,c.centerX+std::cos(a)*r,c.centerZ+std::sin(a)*r,2.2f+Random(state)*1.4f,a);
        }
        if (c.ruin) {
            for (int i=-1;i<=1;i+=2)
                Box(c.scenery,{c.centerX+i*1.25f-.25f,Ground,c.centerZ},{.5f,2.8f,.65f},{.64f,.68f,.58f});
            Box(c.scenery,{c.centerX-1.5f,Ground+2.8f,c.centerZ},{3,.45f,.65f},{.78f,.79f,.65f});
            Cone(c.scenery,{c.centerX,Ground,c.centerZ+1.8f},.55f,1,{.34f,.79f,.77f},4);
        } else {
            Cone(c.scenery,{c.centerX,Ground,c.centerZ},c.radius*.38f,2,{.49f,.60f,.36f},7);
        }
    }
    return c;
}
void World::Update(const WorldPosition& p,int radius)
{
    ...
    if (m_radius==radius && m_center==center) return;
    ...
    for (int z=-radius;z<=radius;++z)
        for (int x=-radius;x<=radius;++x) {
            ...
            if (m_chunks.find(key)==m_chunks.end()) m_chunks.emplace(key,Generate(key.first,key.second));
        }
}
...
bool World::CanWalk(const WorldPosition& p) const
{
    ...
    if (Dock(p,.18f)) return true;
    if (!LandAt(p,-.3f)) return false;
    const auto c=At(p);
    if (c->harbor) {
        for (auto h:Houses)
            if (p.x>h.x-.25 && p.x<h.x+h.w+.25 && p.z>h.z-.25 && p.z<h.z+h.d+.25) return false;
        if (p.x>19.55 && p.x<21.45 && p.z>12.25 && p.z<14.15) return false;
    } else {
        const float dx=static_cast<float>(p.x)-c->centerX,dz=static_cast<float>(p.z)-c->centerZ;
        if (!c->ruin && dx*dx+dz*dz < c->radius*c->radius*.16f) return false;
        if (c->ruin && std::abs(dz)<.95f && std::abs(std::abs(dx)-1.25f)<.55f) return false;
    }
    return true;
}
bool World::CanSail(const WorldPosition& p) const
{
    if (!At(p) || LandAt(p,.2f) || Dock(p,-.1f)) return false;
    for (int i=0;i<12;++i) {
        WorldPosition edge=p;
        edge.Move(1.2*std::cos(2*Pi*i/12),1.2*std::sin(2*Pi*i/12));
        if (!At(edge) || LandAt(edge,.15f) || Dock(edge,0)) return false;
    }
    return true;
}
void World::Render(Renderer& r,const WorldPosition& camera,float time,bool grid) const
{
    ...
    Mesh water;
    Quad(water,{-seaExtent,-.04f,-seaExtent},{seaExtent,-.04f,-seaExtent},
        {seaExtent,-.04f,seaExtent},{-seaExtent,-.04f,seaExtent},{.055f,.40f,.51f});
    r.Submit(water);
    for (const auto& entry:m_chunks) {
        WorldPosition origin;origin.chunkX=entry.first.first;origin.chunkZ=entry.first.second;origin.x=0;origin.z=0;
        ...
        for (int z=0;z<8;++z) for (int x=0;x<8;++x) {
            ...
            Quad(surface,{px,0,pz},{px+1.1f+light*.7f,0,pz},
                {px+1.3f+light*.7f,0,pz+.065f},{px+.2f,0,pz+.065f},
                {.09f+light*.055f,.47f+light*.08f,.57f+light*.07f});
        }
        if (grid) {
            Quad(surface,{0,.65f,0},{32,.65f,0},{32,.65f,.035f},{0,.65f,.035f},{.98f,.71f,.29f});
            Quad(surface,{0,.65f,0},{.035f,.65f,0},{.035f,.65f,32},{0,.65f,32},{.98f,.71f,.29f});
        }
